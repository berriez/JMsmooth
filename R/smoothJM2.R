smoothJM2 <- function(formula, data, survival, maxiter = 200, rightskewed = c(T,T), n1 = 3, n2 = 3, eps = 0.0001, verbose = FALSE)
{
  nclas <- n1*n2
  ...
  print(all.vars(survival))
  print(all.vars(formula))
  form_vars <- all.vars(formula)
  survnames <- all.vars(survival)

  print(c(all.vars(survival),all.vars(formula)))
  m2 <- smoothmixed(formula, data =  data, n1 = 3, n2 = 3, maxiter = 1)
  ...
  for (iter in 1:maxiter)
  {
    ...
    for(i in 1:nclas){
      ...
      #   t1 <- weights[idx_start:idx_end] # creates a vector with weights replaces matrix... to handle unbalanced design
      #  xv1 <- sweep(xtil[idx_start:idx_end,], 1, weights[idx_start:idx_end], FUN="*")
  ...
  res <-list(best = lcjmFit$best, n1 = n1, n2 = n2, crosstable = crosstable, data = data, f0 = f0,
             ...
             nclas = nclas,
             subjmat = subjmat, nscoef = nscoef, data = data, survival = survival,
             yp = yp, nrow = nrow, p = p, ind = ind, nobs = nobs, btil = btil, sd = sd, niter = iter,
             skew = rightskewed, jlcmm = lcjmFit, survmodel = fit1$coef,
             survdata = survdata)
